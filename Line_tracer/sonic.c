...
{
   timer1A_delayus(800);   // Wait 800 ms 1.25Hz
   GPIO_PORTB_DATA_R |= 0x02;               // Start Trigger High
   timer1A_delayus(10);      // Wait 10 us
   GPIO_PORTB_DATA_R &= 0x00;               // Set Trigger Low
}
...
void Init_PortD(void){
  ...
  GPIO_PORTB_DIR_R |= 0x02;         // PE1 output
  GPIO_PORTB_AFSEL_R &= ~0x02;      // not alternative
  GPIO_PORTB_AMSEL_R &= ~0x02;      // no analog
  GPIO_PORTB_DEN_R |= 0x02;         // enable PE1
	GPIO_PORTB_DATA_R &= 0x00;                            // Start Trigger Low
}
